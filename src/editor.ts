...
export class AreaCardEliteEditor extends LitElement {
  ...
  private _getDeviceClasses(domain: string): string[] {
    ...
    // Get entities from the selected area
    const entities = Object.entries(this.hass.states || {})
      .filter(([entityId, entity]) => {
        // Check if entity belongs to selected area
        const entityDomain = entityId.split(".")[0];
        if (entityDomain !== domain) return false;
        
        // Check area_id in attributes first
        if (entity.attributes?.area_id === this._config?.area) {
          return true;
        }
        
        // Also check entity registry if available
        // This is a fallback for entities that might not have area_id in attributes
        return false;
      })
      .map(([entityId, entity]) => entity);

    console.log(`Found ${entities.length} ${domain} entities in area ${this._config.area}`);
    ...
    console.log(`Device classes for ${domain}:`, uniqueClasses);
      ...
      <div class="card-config">
        ...
        <ha-expansion-panel header="Card Features" outlined>
          <div class="content">
                ...
                .selector=${{
                  select: {
                    ...
                    options: [
                      { 
                        value: "toggle-all", 
                        label: "Toggle All Lights/Switches",
                        description: "Show button to turn all lights and switches on/off"
                      }
                    ]
                  }
                }}
            ...
            ${(this._config.features || []).length > 0 ? html`
              <div class="option">
                <ha-selector
                  .hass=${this.hass}
                  .selector=${{
